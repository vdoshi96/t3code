...
const DEFAULT_CUSTOM_DESKTOP_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), customIdentity.homeDirName),
);
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    const baseDir =
      env.T3CODE_HOME ??
      (yield* input.mode === "dev:desktop" ? DEFAULT_CUSTOM_DESKTOP_HOME : DEFAULT_T3_HOME);
