...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("uses the custom desktop packaging product name for all channels", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Code Custom");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Code Custom");
  });
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    ...
    assert.deepStrictEqual(configuration, {
      appId: "com.vdoshi.t3code.custom",
      teamId: "ABC1234567",
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.vdoshi.t3code.custom</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.vdoshi.t3code.custom");
