...
- The custom desktop dev launcher keeps its separate identity and state under
  `~/.t3code-custom/dev`; see `docs/custom-fork.md`.
...
- Desktop production windows load the bundled UI from
  `t3code-custom://app/index.html` (not a `127.0.0.1` document URL).
